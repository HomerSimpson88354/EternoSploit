    ...
    def check_for_updates_on_startup():
        ...
        def fetch_updates():
            ...
            try:
                response = requests.get(github_updates_url, timeout=10)
                if response.status_code == 200:
                    new_updates = response.text.splitlines()
                    if new_updates != updates_list:
                        updates_list = new_updates
                        messagebox.showinfo("New Updates", "New updates are available! Check the updates box for details.")
                        updates_display.delete("1.0", tk.END)
                        for update in updates_list:
                            updates_display.insert(tk.END, update + "\n")
            except requests.exceptions.RequestException:

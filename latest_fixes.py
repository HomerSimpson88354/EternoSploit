    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if last_fixes_hash is not None and last_fixes_hash == new_hash:
                messagebox.showinfo("Up to Date", "You are already up to date with the latest fixes. No changes detected.")
                print("Debug: Hashes match, no update needed")
                return
            # If hashes differ or no previous hash exists, proceed with update
            with open(local_fixes_path, 'w', encoding='utf-8') as f:
                f.write(new_content)
            messagebox.showinfo("Update Available", "New or changed code fixes have been detected. Applying updates now...")
            print("Debug: New or changed fixes detected, proceeding to apply")
...
# Function to check and apply fixes on startup
def check_and_apply_fixes_on_startup():
    def perform_check():
        global last_fixes_hash
        github_fixes_url = "https://raw.githubusercontent.com/HomerSimpson88354/EternoSploit/main/latest_fixes.py"
        local_fixes_path = os.path.join(os.getcwd(), "latest_fixes.py")
        ...
        try:
            ...
            if response.status_code == 200:
                new_content = response.text
                new_hash = compute_hash(new_content)
                print(f"Debug (Startup): Current stored hash = {last_fixes_hash}, New content hash = {new_hash}")
                if last_fixes_hash is None or last_fixes_hash != new_hash:
                    # Changes detected or no previous hash, apply updates automatically
                    with open(local_fixes_path, 'w', encoding='utf-8') as f:
                        f.write(new_content)
                    print("Debug (Startup): Changes detected or no previous hash, applying updates")
                    apply_fixes(local_fixes_path, new_hash)
                else:
                    print("Debug (Startup): No changes detected on startup")
            else:
                print(f"Debug (Startup): Failed to fetch fixes, status code = {response.status_code}")
        except requests.exceptions.RequestException as e:
            print(f"Debug (Startup): Exception during fetch on startup = {str(e)}")
    thread = Thread(target=perform_check, daemon=True)
    thread.start()
...
apply_theme("Green")
# Schedule both updates and fixes checks on startup
root.after(500, check_for_updates_on_startup)
root.after(1000, check_and_apply_fixes_on_startup)
root.mainloop()

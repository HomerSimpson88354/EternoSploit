...
# Path to store updates temporarily
TEMP_UPDATE_DIR = os.path.join(os.getenv("TEMP"), "EternoSploitUpdates")
TEMP_BINARY_PATH = os.path.join(TEMP_UPDATE_DIR, "EternoSploit_new.exe")
TEMP_SCRIPT_PATH = os.path.join(TEMP_UPDATE_DIR, "EternoSploit_new.py")

def ensure_temp_dir():
    if not os.path.exists(TEMP_UPDATE_DIR):
...
def check_for_updates_auto():
    ...
    try:
        ...
        if response.status_code != 200:
            messagebox.showerror("Error", f"Failed to fetch update info. Status code: {response.status_code}")
            return
        
        version_data = response.json()
        latest_version = version_data.get("version", "0.0")
        updates_list = version_data.get("updates", [])
        ...
        if latest_version > CURRENT_VERSION:
            if messagebox.askyesno("Update Available", f"Version {latest_version} is available (current: {CURRENT_VERSION}). Download now?"):
        ...
        messagebox.showerror("Error", "Failed to parse update information from GitHub.")

def apply_update(new_version):
    """Download and apply the update based on whether it's an EXE or script."""
    ensure_temp_dir()
    is_exe = getattr(sys, 'frozen', False)  # Check if running as compiled EXE (via PyInstaller)

    if is_exe:
        if download_file(BINARY_UPDATE_URL, TEMP_BINARY_PATH):
            current_exe = sys.executable
            script = os.path.join(TEMP_UPDATE_DIR, "replace_exe.bat")
            with open(script, 'w') as f:
                f.write(f"""
@echo off
timeout /t 2 /nobreak > nul
copy "{TEMP_BINARY_PATH}" "{current_exe}" /Y
start "" "{current_exe}"
del "{TEMP_BINARY_PATH}"
del "%~f0"
""")
            messagebox.showinfo("Update Downloaded", f"Version {new_version} downloaded. The application will restart to apply the update.")
            subprocess.Popen([script], shell=True)
            sys.exit(0)
    else:
        if download_file(SCRIPT_UPDATE_URL, TEMP_SCRIPT_PATH):
            current_script = os.path.abspath(__file__)
            shutil.copy2(TEMP_SCRIPT_PATH, current_script)
            os.remove(TEMP_SCRIPT_PATH)
            messagebox.showinfo("Update Applied", f"Version {new_version} applied. Restart the application to see changes.")
            sys.exit(0)
...
def apply_theme(theme_name):
    ...
    all_buttons = [select_folder_btn, load_btn, infinite_yield_btn, owl_hub_btn, bloodyv2_btn, ruhub_btn, rivals_btn, brookhaven_btn,
                   attach_btn, execute_btn, kill_btn, open_btn, save_btn, settings_btn, back_btn, check_updates_btn] + theme_buttons
...
def check_for_updates_on_startup():
    ...
    root.after(500, check_for_updates_auto)

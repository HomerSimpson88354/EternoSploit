    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if last_fixes_hash == new_hash:
                ...
                return
            # Content differs, save and proceed to apply
            with open(local_fixes_path, 'w', encoding='utf-8') as f:
                f.write(new_content)
            messagebox.showinfo("Update Available", "New code fixes have been detected. Applying updates and restarting...")
...
def apply_fixes(fixes_path, new_hash):
    try:
        with open(fixes_path, 'r', encoding='utf-8') as f:
            content = f.read()
        # Save the new hash to ensure we don't re-download the same update
        save_last_fixes_hash(new_hash)
        print(f"Debug: Saved hash {new_hash} before applying fixes")
        if "requires_restart = True" in content:
            # If restart is explicitly required, notify and restart
            messagebox.showinfo("Restarting", "Updates downloaded. Restarting EternoSploit to apply changes...")
            print("Debug: Restart required, performing automatic restart")
            apply_fixes_on_startup(fixes_path)  # Ensure fixes are marked for startup
            restart_application()
        else:
            # Apply fixes directly if no restart is needed
            exec(content, globals())  # Execute the fixes in the current global namespace
            messagebox.showinfo("Success", "Code fixes applied successfully.")
            print(f"Debug: Fixes applied directly, updated hash to {new_hash}")
    except Exception as e:
        messagebox.showerror("Error", f"Failed to apply code fixes: {str(e)}")
        print(f"Debug: Error during apply = {str(e)}")

def apply_fixes_on_startup(fixes_path):
    """Mark fixes to be applied on startup if restart is needed."""
    pending_fixes_file = os.path.join(os.getcwd(), "pending_fixes.txt")
    try:
        with open(pending_fixes_file, 'w', encoding='utf-8') as f:
            f.write(fixes_path)
        print("Debug: Marked fixes for startup application")
    except Exception as e:
        print(f"Debug: Failed to mark fixes for startup: {str(e)}")

def check_and_apply_pending_fixes():
    """Check for pending fixes on startup and apply them."""
    pending_fixes_file = os.path.join(os.getcwd(), "pending_fixes.txt")
    if os.path.exists(pending_fixes_file):
        try:
            with open(pending_fixes_file, 'r', encoding='utf-8') as f:
                fixes_path = f.read().strip()
            if os.path.exists(fixes_path):
                with open(fixes_path, 'r', encoding='utf-8') as f:
                    content = f.read()
                print("Debug: Applying pending fixes on startup")
                exec(content, globals())  # Apply the fixes by executing in global namespace
                messagebox.showinfo("Success", "Pending code fixes applied successfully after restart.")
                os.remove(pending_fixes_file)  # Remove the pending marker
                print("Debug: Pending fixes applied and marker removed")
            else:
                print(f"Debug: Fixes file {fixes_path} not found on startup")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to apply pending fixes on startup: {str(e)}")
            print(f"Debug: Error applying pending fixes on startup: {str(e)}")
...
def restart_application():
    ...
    try:
        ...
        print(f"Debug: Restart failed: {str(e)}")
